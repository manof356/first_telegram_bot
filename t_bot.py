...
# Todo: Переписать основные функции кнопок без "/".
# Todo: Поправить сообщение о некорректном языке введенного сообщения согласно спряжениям русского языка
...
lang_dict = {"/English": False, "/Русский": True}  # костыльный словарь для работы функции is_rus_eng_letters
...
# Todo: наверно надо поправить, чтобы он работал не из любого состояния
...
async def vigenere_start(message: types.Message):
    ...
    await message.delete()  # наше соообщение удаляется
...
@dp.message_handler(lambda message: not is_rus_eng_letters(message.text, lang_dict[data['lang']]),
...
async def is_phrase_right_alph(message: types.Message):
    await bot.send_message(chat_id=message.chat.id, text=f"Вы выбрали {data['lang'][1:]}, "
                                                         f"а ввели фразу не на {data['lang'][1:]}!")
...
async def is_key_right_alph(message: types.Message):
    await bot.send_message(chat_id=message.chat.id, text=f"Вы выбрали {data['lang'][1:]}, "
                                                         f"а ввели ключ не на {data['lang'][1:]}!")
